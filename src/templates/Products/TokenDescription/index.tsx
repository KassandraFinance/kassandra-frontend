...
const TokenDescription = () => {
  return (
    <>
      ...
      <S.Line />
      <S.Text>
        <p>The Heimdall Social Index (HEIM) tracks the performance of a portfolio composed by selecting the most socially active cryptocurrencies in the last 30 days. This portfolio is weighted according to the values of social score made available by Heimdall and the technology provided by the Kassandra Protocol. The portfolio is a SIP (Smart Index Pool) and is rebalanced by arbitrageurs just like an usual liquidity pool. The index is accompanied by its own token $HEIM, allowing investors to buy the index by purchasing the token, just like an ETF.</p>

        <h2>Goal</h2>
        <p>The index primary goal is to offer exposure to the most popular cryptocurrencies in a simple and easy way. The investor needs only to acquire the index token. With this goal in mind it is paramount to develop a tool which can measure the popularity of a particular crytpo, this tool is the creation of Heimdall and it is the bedrock of the Index.</p>

        <h2>Dynamic Weights</h2>
        <span>The main aspect of the SIP is that the weights are not fixed, 
          but change according to the social score dynamics in order to 
          keep track of the most active tokens.
        </span>
        <p>The frequency and the amount in which the weights for each cryptocurrency inside the SIP are updated directly affect the value of the pool. Since any weight change can offer the opportunity for arbitrage, therefore decreasing the value of the pool. In addition, gas costs must be considered because changing weights means writing new information on the blockchain. The gas costs will be covered using funds from a wallet managed by the DAO and funded by the exit fees collected from the users.</p>
        <span>
